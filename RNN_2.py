...
variables_predictoras = ['INPC_General', 'INPC_Sub']
...
def inv_scaler(y, current_scaler):
    ...
    df.loc[:, ['INPC_General', 'INPC_Sub']] = y
    y_esc = pd.DataFrame(current_scaler.inverse_transform(df), columns=test_scaled.columns)[['INPC_General', 'INPC_Sub']]
    return y_esc
